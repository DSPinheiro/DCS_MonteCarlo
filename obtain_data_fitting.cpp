...
void Obtain_data_fitting::FitData(int numbins, double angle_para, int toint_para, double angle_anti, int toint_anti){

    if(Data_Fit_anti.size() < (unsigned int)numbins)
        Data_Fit_anti.resize(plotparameters.nubins);
    if(Data_Fit_para.size() < (unsigned int)numbins)
        Data_Fit_para.resize(plotparameters.nubins);
