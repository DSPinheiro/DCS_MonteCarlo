...
extern vector<plotresponc> Data_Fit_anti, Data_Fit_para;

extern pick picks[5];

extern double amplitu_con_para, peak_posi_para, amplitu_con_anti, peak_posi_anti, a_para_voig[MA], a_anti_voig[MA], vert_div_corr, refra_corr, middle_energy, d_lat;

extern double FWMH_V_para, FWMH_V_anti;
...
void Analysie_Voigt::analyse(){
    ...
    d_lat_2 = Latice_temp(d_lat, temperature_parameters.T_crystal_2_anti);
    ...
    if(not AnalysiesCrystaltilts.make_an_C1_ta){
        gener_out << endl;
        gener_out << "-----------------------------------------" << endl;
        gener_out << endl;
