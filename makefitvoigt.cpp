...
extern double amplitu_con_para, peak_posi_para, amplitu_con_anti, peak_posi_anti;

double FWMH_V_para, FWMH_V_anti, a_para_voig[MA], a_anti_voig[MA];
...
void MakefitVoigt::fit(bool Parallel){
    ...
    NPT = plotparameters.nubins;

    UserSettings.TrueVoigt = true;
    ...
    if(Parallel){
        for(int i = 0; i < plotparameters.nubins; i++){
